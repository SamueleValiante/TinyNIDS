...
class MultiHeadLinearAttention(layers.Layer):
    ...
    def feature_map(self, x):
        # elu(x)+1: garantisce valori non negativi, richiesti perche' il trucco della linear attention si comporti come dei "pesi" validi
        return tf.nn.elu(x) + 1.0

    def split_heads(self, x):
        # (batch, N, d_model) -> (batch, N, num_heads, head_dim) -1 al posto di tf.shape(x)[0]: la dimensione di batch resta
        # libera senza generare un'operazione SHAPE nel grafo
        seq_len = x.shape[1]
        x = tf.reshape(x, (-1, seq_len, self.num_heads, self.head_dim))
        return x

    def call(self, x):
        q = self.feature_map(self.split_heads(self.wq(x)))
        ...
        numerator = tf.einsum("bnhd,bhde->bnhe", q, kv)
...
def build_model(seq_len=20, input_dim=16, d_model=64, ff_dim=128,
    ...
    # batch_size=None (default): usato per training/valutazione normali, la dimensione di batch resta dinamica
