...
static volatile uint64_t g_latency_sum_us = 0;
static volatile uint32_t g_latency_min_us = UINT32_MAX;
static volatile uint32_t g_latency_max_us = 0;
...
static void runInference() {
  ...
  uint32_t latency_us = (uint32_t)(t1 - t0);
  g_inference_count++;
  g_latency_sum_us += latency_us;
  if (latency_us < g_latency_min_us) g_latency_min_us = latency_us;
  if (latency_us > g_latency_max_us) g_latency_max_us = latency_us;
...
static void inferenceTask(void *pv) {
  ...
  while (true) {
    if (xQueueReceive(packetQueue, &pkt, portMAX_DELAY) == pdTRUE) {
      g_packets_received++;
#if VERBOSE_PACKET_DEBUG
      if (g_packets_received <= 10 || g_packets_received % 20 == 0) {
        Serial.printf("[DEBUG] pacchetto #%u: proto=%d len=%d src=%d.%d.%d.%d dst=%d.%d.%d.%d\n",
                      (unsigned)g_packets_received, pkt.protocol, pkt.len,
...
static void monitorTask(void *pv) {
  ...
  while (true) {
    ...
    UBaseType_t queue_backlog = packetQueue ? uxQueueMessagesWaiting(packetQueue) : 0;

    uint64_t inf_count = g_inference_count;
    uint32_t avg_latency = (inf_count > 0) ? (uint32_t)(g_latency_sum_us / inf_count) : 0;
    uint32_t min_latency = (inf_count > 0) ? g_latency_min_us : 0;
    uint32_t max_latency = (inf_count > 0) ? g_latency_max_us : 0;

    Serial.printf("\n[METRICHE] uptime=%llus pacchetti_ricevuti=%u pacchetti_scartati=%u coda_in_attesa=%u\n",
                  (unsigned long long)(esp_timer_get_time() / 1000000),
                  (unsigned)g_packets_received, (unsigned)g_packets_dropped, (unsigned)queue_backlog);
    Serial.printf("[METRICHE] inferenze_totali=%llu  latenza_us: min=%u avg=%u max=%u\n",
                  (unsigned long long)inf_count, (unsigned)min_latency, (unsigned)avg_latency,
                  (unsigned)max_latency);
    Serial.printf("[METRICHE] heap_libero=%u byte  heap_minimo_storico=%u byte  tensor_arena=%d byte\n\n",
                  (unsigned)heap_free, (unsigned)heap_min,
                  interpreter ? (int)interpreter->arena_used_bytes() : -1);
  }
}
...
void snifferCallBack(void *buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (etherType == 0x0806 && ATTACK_MODE != 1) {
    ...
    if (xQueueSend(packetQueue, &rp, 0) != pdTRUE) {   // non bloccante: se la coda e' piena, scarta
      g_packets_dropped++;
    }
  ...
  rp.len = (uint16_t)len;
  rp.timestamp_us = esp_timer_get_time();
  if (xQueueSend(packetQueue, &rp, 0) != pdTRUE) {
    g_packets_dropped++;
  }
}
